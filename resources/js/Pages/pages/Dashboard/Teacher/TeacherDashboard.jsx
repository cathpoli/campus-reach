...
export default function TeacherDashboard({ hasSchedule = false, schedules = [], events = [] }) {
	...
	const handleCancelAppointment = (eventId) => {
		Swal.fire({
			...
			text: 'Are you sure you want to cancel this appointment? The student will be notified.',
			icon: 'warning',
			...
			if (result.isConfirmed) {
				router.post(`/appointment/${eventId}/cancel`, {}, {
					onSuccess: () => {
						Swal.fire({
							title: 'Cancelled!',
		...
		const swalOptions = {
			...
			html: `
				<div class="text-center">
					<p class="mb-4">${event.student_name} booked an appointment on ${startTimeStr} to ${endTime}</p>
					${event.zoom_link ? `
	...
	useEffect(() => {
		// Show reminder alert if teacher has no schedule
		if (!hasSchedule) {
			Swal.fire({
				title: 'Reminder!',
				html: "You don't have any schedule for this week.<br>Please set up your schedule.",
				icon: 'info',
				showCancelButton: true,
				confirmButtonColor: '#00bcd4',
				cancelButtonColor: '#6b7280',
				confirmButtonText: 'Setup Schedule',
	...
	return (
		<AppLayout>
			<PageMeta
				title="Teacher Dashboard - Campus Reach"
				description="Campus Reach Teacher Dashboard"
			/>
											...
											{day && (
												<>
													...
													{dayEvents.map((event) => (
														...
														title={`${event.title} - ${event.student_name}`}
